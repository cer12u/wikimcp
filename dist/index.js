...
`;
function isListPagesArgs(args) {
    return (typeof args === "object" &&
        args !== null);
}
function isGetPageArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        (("id" in args && typeof args.id === "number") ||
            ("path" in args && typeof args.path === "string")));
}
function isCreatePageArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "path" in args &&
...
function isUpdatePageArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        (("id" in args && typeof args.id === "number") ||
            ("path" in args && typeof args.path === "string")));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        if (name === "wiki_list_pages") {
            ...
            const pageList = pages.map((page) => `- ${page.title} (ID: ${page.id}, Path: ${page.path})${page.updatedAt ? `\n  Last Updated: ${new Date(page.updatedAt).toLocaleString()}` : ''}`).join('\n\n');
            return {
                content: [{
                        type: "text",
